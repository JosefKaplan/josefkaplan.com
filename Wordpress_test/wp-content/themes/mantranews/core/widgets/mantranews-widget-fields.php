...
function mantranews_widgets_show_widget_field( $instance = '', $widget_field = '', $mb_field_value = '' ) {
	...
	switch ( $mantranews_widgets_field_type ) {
		...
		case 'text' :
			...
			<p>
				<label
					for="<?php echo esc_attr( $instance->get_field_id( $mantranews_widgets_name ) ); ?>"><?php echo esc_html( $mantranews_widgets_title ); ?>
					:</label>
				...
				<?php if ( isset( $mantranews_widgets_description ) ) { ?>
					<br/>
					<small><?php echo esc_html( $mantranews_widgets_description ); ?></small>
				<?php } ?>
			</p>
			<?php
			break;
		...
		case 'checkbox' :
			...
			<p>
				...
				<?php if ( isset( $mantranews_widgets_description ) ) { ?>
					<br/>
					<small><?php echo wp_kses_post( $mantranews_widgets_description ); ?></small>
				<?php } ?>
			</p>
			<?php
			break;
		...
		case 'radio' :
			...
			<div class="radio-wrapper">
				<?php
				foreach ( $mantranews_widgets_field_options as $athm_option_name => $athm_option_title ) {
					?>
		...
		case 'select' :
			...
			<p>
				<label
					for="<?php echo esc_attr( $instance->get_field_id( $mantranews_widgets_name ) ); ?>"><?php echo esc_html( $mantranews_widgets_title ); ?>
					:</label>
				<select name="<?php echo esc_attr( $instance->get_field_name( $mantranews_widgets_name ) );
				if ( $is_multiple ) {
					echo '[]';
				} ?>"
				        id="<?php echo esc_attr( $instance->get_field_id( $mantranews_widgets_name ) ); ?>"
		...
		case 'widget_section_header':
			?>
			<span class="section-header"><?php echo esc_html( $mantranews_widgets_title ); ?></span>
			<?php
			break;
		...
		case 'widget_layout_image':
			...
			<div class="layout-image-wrapper">
				<span class="image-title"><?php echo esc_html( $mantranews_widgets_title ); ?></span>
				<img src="<?php echo esc_url( $mantranews_widgets_layout_img ); ?>"
				     title="<?php echo esc_attr__( 'Widget Layout', 'mantranews' ); ?>"/>
			</div>
			<?php
			break;
		...
		case 'upload' :
			...
			$output .= '<label for="' . esc_attr( $instance->get_field_id( $mantranews_widgets_name ) ) . '">' . esc_html( $mantranews_widgets_title ) . '</label><br/>';
			$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . esc_html__( 'No file chosen', 'mantranews' ) . '" />' . "\n";
			if ( function_exists( 'wp_enqueue_media' ) ) {
				if ( ( $value == '' ) ) {
					$output .= '<input id="upload-' . $id . '" class="wid-upload-button button" type="button" value="' . esc_html__( 'Upload', 'mantranews' ) . '" />' . "\n";
				} else {
					$output .= '<input id="remove-' . $id . '" class="wid-remove-file button" type="button" value="' . esc_html__( 'Remove', 'mantranews' ) . '" />' . "\n";
				}
			} else {
				$output .= '<p><i>' . esc_html__( 'Upgrade your version of WordPress for full media support.', 'mantranews' ) . '</i></p>';
			}

			$output .= '<div class="screenshot upload-thumb" id="' . $id . '-image">' . "\n";

			if ( $value != '' ) {
				$remove = '<a class="remove-image">' . esc_html__( 'Remove', 'mantranews' ) . '</a>';
				$image  = preg_match( '/(^.*\.jpg|jpeg|png|gif|ico*)/i', $value );
				if ( $image ) {
					$output .= '<img src="' . esc_url( $value ) . '" alt="' . esc_attr__( 'Upload image', 'mantranews' ) . '" />';
				} else {
					$parts = explode( "/", $value );
					...
					$title = esc_html__( 'View File', 'mantranews' );
...
function mantranews_widgets_updated_field_value( $widget_field, $new_field_value ) {


	$mantranews_widgets_field_type = '';
	...
	switch ( $mantranews_widgets_field_type ) {
		...
		case 'number':
			return mantranews_sanitize_number( $new_field_value );
			break;
		// Allow some tags in textareas
		case 'textarea':
			$mantranews_widgets_allowed_tags = array(
				'p'      => array(),
				'em'     => array(),
				'strong' => array(),
				'a'      => array(
			...
			return wp_kses( $new_field_value, $mantranews_widgets_allowed_tags );
			break;
