 ...
 * @access public
 */
final class envince_Custom_Colors {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_envince_editor_styles',         array( $this, 'editor_styles_callback' ) );
		add_action( 'wp_ajax_no_priv_envince_editor_styles', array( $this, 'editor_styles_callback' ) );
	}
	...
	public function get_primary_styles() {
		...
		$style .= "input[type='submit'], input[type='reset'], input[type='button'], button,
				...
				.scroll-to-top:hover, .btn-default, .label-default
		...
		/* border-bottom-color for navbar */
		$style .= ".navbar.navbar-default { border-bottom-color: #{$hex}; }";
		...
		$style .= "body, .navbar .navbar-default, .widget-title, #comments-number, #reply-title,
		...
		/* border-top-color */
		$style .= ".breadcrumb-trail, .navbar-nav > li > .dropdown-menu { border-top-color: rgba( {$rgb}, 0.85 ); } ";
	...
	public function customize_register( $wp_customize ) {
		...
		$wp_customize->add_control(
			new WP_Customize_Color_Control(
				...
				array(
					'label'    => esc_html__( 'Primary Color', 'envince' ),
					'section'  => 'colors',
...
envince_Custom_Colors::get_instance();